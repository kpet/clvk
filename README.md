...
If you already have a perfetto library in your system, you still need to define
the source directory to let `clvk` find `perfetto.h`.
...
* `CLVK_PERFETTO_MAX_TRACE_SIZE` specifies the maximum size (in kB) of traces
  generated by `perfetto`. It only applies when using `perfetto` with the
  `InProcess` backend.

* `CLVK_PERFETTO_DEST_TRACE` specifies the filename to use for the traces
