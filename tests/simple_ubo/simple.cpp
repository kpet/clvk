...
int main(int argc, char* argv[])
{
    ...
    int c_data[num_items];
    for (auto i = 0; i != num_items; ++i) {
      c_data[i] = 1;
    }
    auto c_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, BUFFER_SIZE, nullptr, &err);
    CHECK_CL_ERRCODE(err);

    err = clEnqueueWriteBuffer(queue, c_buffer, CL_TRUE, 0, BUFFER_SIZE, c_data, 0, nullptr, nullptr);
    CHECK_CL_ERRCODE(err);
