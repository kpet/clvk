...
struct cvk_mem : public _cl_mem, api_object<object_magic::memory_object> {
    ...
    bool CHECK_RETURN map() { return map_and_invalidate(0, m_size); }
    bool CHECK_RETURN map_write_only() { return map_and_invalidate(0, 0); }
    bool CHECK_RETURN map_to_read(VkDeviceSize offset, VkDeviceSize size) {
        return map_and_invalidate(offset, size);
    }
    void unmap() { unmap_and_flush(0, m_size); }
    void unmap_read_only() { unmap_and_flush(0, 0); }
    void unmap_to_write(VkDeviceSize offset, VkDeviceSize size) {
        unmap_and_flush(offset, size);
    }
...
private:
    bool CHECK_RETURN map_and_invalidate(VkDeviceSize offset,
                                         VkDeviceSize size);
    void unmap_and_flush(VkDeviceSize offset, VkDeviceSize size);
